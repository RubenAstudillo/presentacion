...
es un cubrimiento universal. Sea \(p : \tilde X \to X,\ \tilde X\) un
...
\[ \tilde \gamma \, (0) = \tilde x_1, \quad \tilde \gamma \, (1) =
  \tilde x_2 = \phi ([\gamma]) \]
para algún \(\tilde x_1 \in p^{-1} (x_0)\). La ecuación
\eqref{hip:lifting} aplicada a estos \(\tilde x_0, \tilde x_1,\) nos
...
\begin{teorema}
  ...
  arco-conexo de un espacio \(X\) arco-conexo, localmente arco-conexo.
  Sea \(H\) el subgrupo
  \[ H := p_* \left( \pi (\tilde X, \tilde x_0) \right) \subseteq \pi
    \left( X , x_0 \right) \]
  Entonces \(G (\tilde X)\) es isomorfo a \(\pi (X, x_0) / H\) con \(N
  (H)\) el normalizador de \(H\) en \(\pi (X , x_0)\)
\end{teorema}
\begin{proof}
  La idea de la demostración sera el primer teorema del isomorfismo para
  grupos. Sea \(L\) la función definida en la ecuación \eqref{def:L}
  pero restringida a \(L : N(H) \subseteq \pi (X, x_0) \to G ( \tilde
  X)\). Hemos de probar que este es un homomorfismo. Sean
  \([\gamma],[\gamma'] \in \pi (X , x_0)\) dos curvas arbitrarias. Sean
  \(\tilde \gamma , \tilde{\gamma '}\) los levantamientos respectivos de
  los representantes de clases, los cuales cumplen las siguientes
  ecuaciones
  \begin{gather*}
  ...
  Dado que la transformación de deck
  \[ \tau \circ \tau : (\tilde X, \tilde x _0) \to (\tilde X , \tau
    \circ \tau' \, (\tilde x_0))
  \]
  también envía al punto \(\tilde x_0\) a \(\tau \circ \tau' \, (\tilde
  x_0)\) , que las transformaciones de \emph{deck} son levantamientos
  (demostración teorema \ref{thm:exists-isomor}) y que la composición de
  levantamientos es un levantamiento podemos aplicar el
