...
\pi \end{bmatrix} \) con \( 0 \sim 10 \pi \)] \label{ej:10pi}
  ...
  si mismo mediante la identificación \(0 \sim 2 \pi\). Podemos imaginar
  ...
  Como acotación, notar que si nuestro cubrimiento fuera \([0, 10 \pi]\)
...
tiene la siguiente definición auxiliar
...
\begin{proof}
  ...
  \(A^c\). Por tanto, este también es un conjunto abierto. Obtenemos así
...
\begin{ejemplo}[Cubrimiento de toro] \label{ej:cubrimiento-toro}
  ...
  \end{figure}
  La función en este caso correspondería a
  \begin{gather*}
    \tilde X = [0,3] \times [0,2] \, / \, \{
  ...
  cubrimiento que también tiene identificaciones de igual manera.
\end{ejemplo}
...
\begin{proof}
  ...
  que es cubierta (definición \ref{def:cubierta}). Por tanto
  \[ X \subseteq \bigcup_{x \in X} U_x\]
  con \(U_x\) siendo las vecindades cubiertas para cada \(x\). De lo
  anterior, se obtiene la siguiente inclusión
  ...
  se tendría una contradicción en decir que \((p \mid_{V_0})\) es un
...
\begin{ejemplo}[Continuación ejemplo \ref{ej:10pi}]
  ...
  homomorfismo inducido para \(p\) corresponde a la función
  \begin{align*}
  ...
  \(5 \mathbb Z \) que es isomorfo (ambos tienen un único generador) a
  \(\mathbb Z\).
\end{ejemplo}
...
comportamiento de una función conocida como el levantamiento derivado.
\begin{definicion}[Levantamiento derivado] \label{def:levantamiento-derivado}
  ...
  \end{align*}
  % TODO: no sé si usar el nombre de munkress
...
transformaciones de \emph{deck}\footnote{En español seria transformaciones de
  cartas, pero así podemos utilizar un nombre usual de \cite{Hatcher}}.
...
\begin{teorema}\label{thm:lifting-theorem-general}
  ...
  \(f : (Y , y_0) \to ( X , x _0 )\) una función continua con \(Y\)
...
\begin{proof}
  ...
  % No entiendo bien lo que me quiere decir mariela
...
\begin{proof}
  ...
  \(\emptyset\). Pero dada la hipótesis de que existe un punto donde los
  levantamientos coinciden, \(K\) no puede ser vació y por tanto es el
  espacio total \(Y\).
\end{proof}
...
\begin{teorema}\label{thm:exists-isomor}
  ...
  X\) son isomorficos vía un isomorfismo \(f : \tilde X_1 \to \tilde
...
\end{teorema}
\begin{proof}
  Se inicia en la dirección \(\Rightarrow\). Un isomorfismo de cubrimientos
  ...
  llegada sea un cubrimiento. De la primera ecuación se obtiene que
  \[ p_{1,*} = \left( p_2 \circ f \right)_* \]
  que por definición al ser aplicadas a \([\gamma] \in \pi (\tilde X_1,
  \tilde x_1)\)
  \begin{equation*}
      p_{1,*} \left( [\gamma] \right) = [p_1 \circ \gamma] = [p_2 \circ
      f \circ \gamma ] = p_{2,*} \left( [f \circ \gamma] \right)
  \end{equation*}
  lo que junto la biyectividad de \(f\) implican el resultado.

  Para la dirección \(\Leftarrow\), supongamos que \(p_{1,*} (\pi
  ...
  del levantamiento obteniendo así que
...
\ref{def:deck}. Mas aun, \(\varphi\) es el único isomorfismo posible
entre \((\tilde X , \tilde x_1) \to (\tilde X , \tilde x_2 )\) pues de
haber otro, tendría que compartir el punto \(\tilde x_2\) con
\(\varphi\) y iría en contra el Teorema \ref{thm:uniq-general} de
unicidad del levantamiento.

La relación anterior entre \([\gamma]\) y \(\varphi\) es así funcional.
...
\begin{proof}
  ...
  \(L : \pi (X, x_0) \to G ( \tilde X)\), la cual es función por la simple
...
``decisión informada'' para dotarle estructura de grupo. Para ver el
...
\begin{ejemplo}[Grupo fundamental plano proyectivo]
  ...
  Cabe notar que el espacio \((S^2, x_0)\) es simplemente conexo. Esto
  podrá verse formalmente en el ejemplo \ref{ej:S2-simpleconexo} como
  consecuencia del teorema de \vank , pero
  intuitivamente si uno considera a \(S^2\) como una bola, cualquier
  curva puede retraerse homotópicamente por la superficie de esta, siendo
  todas homotópicamente equivalente al punto inicial. Luego \(S^2\) es
...
\subsubsection{Categoría de cubrimientos}
...
\end{definicion}
\begin{acotacion}
  Los elementos terminal e inicial son único modulo isomorfismo.
\end{acotacion}
...
el elemento terminal por definición. Lo que no es tan claro es la
...
\begin{proof}
  ...
  \((Z, z_0)\) que cubre a \((X, x_0)\) mediante la aplicación
  ...
  \ref{thm:uniq-general} de unicidad del levantamiento. Obtenemos así el
  resultado.
\end{proof}
...
\begin{figure}[h]
  ...
  \caption{caracterización categoría}
\end{figure}
