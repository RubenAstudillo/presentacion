...
caminos. Veremos tambien que bajo hipotesis sensatas, este \(\tilde f\)
es unico y refleja informacion homotopica. Para esto necesitamos
...
\begin{proof}
  ...
  Por otro lado \(f\) es una funcion continua sobre \(X\) un espacio
  compacto, por lo tanto alcanza un minimo; a este le denotaremos como
...
si no tambien preservar las homotopias de \(X\) en \(\tilde X\).
\begin{corolario}
...
\begin{proof}
  ...
  contenido en un conjunto cubierto parmente en \(X\), esto utilizando
  el lema de Lebesgue al igual que en la demostracion anterior.
  ...
  que son los bordes inferior y izquierdos del rectangulo.
  ...
  esta definido bajo \(\tilde F\), existe un unico \(V_0 \in
  ...
  Estan definidos en \(\tilde F\) y pueden ser utilizados para seguir
  definiendola inductivamente sobre \(I \times I\).
\end{proof}

Ahora es mas clara la forma de computar grupos fundamentales con
