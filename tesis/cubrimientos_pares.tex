...
(X, x_0)\). Necesitamos algunas definiciones para esto
...
\begin{acotacion}
  Este mapeo no depende del representante de clase \([f] \in \pi \left(
    X, x_0 \right)\) pues las homotopías tiene fijos los extremos.
\end{acotacion}
Una idea geométrica de lo que sucede en los arcos del espacio base
\(\left(X, x_o \right)\) al levantarse mediante el teorema
\ref{thm:lifting-theorem} al espacio cubrimiento \(\left( \tilde X ,
\tilde x _0 \right)\) es que transforma arcos con el mismo punto inicial
y final a arcos donde esto no es necesario. Es decir es posible que
``desenrolle'' los arcos al levantarse. Sin embargo para cumplir la
propiedad de los levantamiento, el punto final de este tiene que
pertenecer al conjunto \(p^{-1} \left( \{x_0\} \right)\), es decir la
fibra de \(x_0\). De hecho podemos enfocarnos puramente en el punto al
que permuta el punto final y podríamos determinar totalmente al arco en
la base al que corresponde bajo hipótesis adecuadas. Esto se vera en la
siguiente definición y teorema.
