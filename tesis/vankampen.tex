...
\end{corolario}
\begin{proof}
  En el final de la demostracion anterior, la cadena \(g_1 g_2 \dots
...
\begin{proof}
  ...
  *_\alpha \pi (A_\alpha, x_0)\) donde
  ...
  \end{align*}
  y por tanto podemos reemplazar a esta cadena en cualquier posicion
  donde aparezca en \(*_\alpha \pi (A_\alpha) / N\) pues si \([x] , [y] \in
  *_\alpha \pi(A_\alpha)\) arbitrarios (particion izquierda y derecha de
  ...
  \Gamma_k\) es una homotopia entre \(F \circ \gamma_k\) e \(F \circ
  \gamma_{k+1}\).
...
Calcularemos algunos ejemplos que utilizen el teorema de \vank desde
...
la wedge-sum \(S^1 \vee S^1\).
  ...
  hay inyecciones dos canonicas de \(S^1\) a \(S^1 \vee S^1\). Por tanto
...
El producto libre de grupos \(*_\alpha G_\alpha\) es en la categoria
\(\mathcal{Grp}\) el co-producto categorico. Esto se ve siguiento la
definicion \ref{def:coproducto} de co-producto, sean \(X,Y,C\) tres grupos
tales que existan homomorfismo \(\phi_1 : X \to C\) y \(\phi_2 : X \to
C\). Ya se habia anteriormente como el producto libre \(X * Y\) admite
una extension (unica) de los homomorfismos \(\phi_1, \phi_2\) a la cual
...
\begin{ejemplo}[Botella de Klein]
  ...
  este conjunto es convexo y todos los caminos son homotopicos a la
  identidad. El grupo fundamental de \(\pi (V, x_0)\) es isomorfo a
  \(\pi (V, y_0)\) con \(y_0\) el vertice (identificado es único) de \(I
  \times I\), esto pues el espacio \(V\) es arco-conexo. Estudiando
  \(\pi (V, y_0)\), es claro que \(V\) es homotopicamente equivalente al
  espacio generado por la curvas \(a\) y \(b\) con punto comun \(y_0\),
  es decir .
  ...
  homomorfismos inducidos por la inclusion
  ...
  b^{-1}]\) por la proyeccion radial desde el centro de \(I \times I\)
  hacia el borde
  \begin{figure}[h]
  ...
  [b^-1]\}\) con la yuxtaposicion como operación, es decir no tiene
...
Sea \(\mathscr{C}\) una categoria. Sean \(X,Y,Z\) objetos en
...
\to P\) son morfismos de la categoria \(\mathscr{C}\) tales que sean
...
categoria \(\mathscr{Grp}\) luce como el siguiente diagrama.
\begin{figure}[h]
...
traves de este functor se guia a revisar la referencia \cite{brown}.
