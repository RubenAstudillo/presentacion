...
\end{proof}
El producto libre aparecerá en la versión mas general del teorema de
\vank . Previo a esto, es útil trabajar con una versión reducida del
...
\begin{teorema}[Teorema \vank ]
  ...
  es sobreyectiva. Mas aun, si cada interseccion \(A_\alpha \cap A_\beta
  \cap A_\gamma\) es arco-conexa, entonces el kernel de \(\Phi\) es un
  subgrupo normal \(N\) de la forma
  \[
    N := \{ i_{\alpha \beta} (\omega) * i_{\beta \alpha} (\omega)^{-1}
