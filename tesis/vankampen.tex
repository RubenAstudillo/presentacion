...
\begin{corolario} \label{cor:vank-trivial}
  ...
  En particular para una wedge-sum \((X \vee Y , {x_0 , y_0})\), dado
  que los singletons tiene grupos fundamentales triviales, se afirma que
  \[ \pi (X \vee Y , \{x_0, y_0\}) \cong \pi (X, x_0) * \pi (Y , y_0) \]
\end{corolario}
...
igualdad pedida en la Definicion \ref{def:coproducto}. Una consecuencia
de \vank es que el functor \(\pi : \mathscr{HoTop}_* \to \mathscr{Grp}\)
preserva co-productos, como fue visto en el ejemplo anterior. Esto
completa la caracterizacion iniciada por el Teorema \ref{thm:prop-prod}
de la relacion del grupo de wedge-sum con sus sumandos.
