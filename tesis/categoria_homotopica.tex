...
El teorema anterior nos una forma trivial de conocer grupos
fundamentales si los espacios a estudiar pueden reducirse a una
estructura producto o co-producto. Un ejemplo sencillo es el siguiente:

\paragraph{Toro :} Una presentacion del torus \(T\) es como espacio cuociente
de \([0,1]^2\). Se identifican los puntos
\begin{gather*}
  (0,t) \sim (1,t), \quad t \in [0,1] \\
  (t,0) \sim (t,1), \quad t \in [0,1]
\end{gather*}
Existe un homeomorfismos entre el toro y el espacio \(S^1 \times S^1\) dado
por la siguiente función \(f\):
\begin{align*}
...
pensamos en el toro como el espacio producto \(S^1 \times S^1\). Por el
teorema \ref{thm:prop-prod}
\[
  \pi \left( T \right) = \pi \left( S^1 \times S^1 \right) = \pi (S^1)
  \times \pi (S^1)
\]
omitiendo el punto de origen puesto que este es un espacio arco-conexo.
