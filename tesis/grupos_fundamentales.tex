...
\begin{definicion}
  ...
  \begin{itemize}
    ...
    \item \(\mathbf {Mor}\) corresponde a una clase de morfismos entre
      elementos de \(\mathbf {Obj}\), tal que en particular \(\forall A \in
      \mathbf {Obj},\ \exists \mathcal i _A : A \to A\).
    \item \((\circ)\) es una operación composición entre morfismos
      asociativa y que cuya identidad para cada objeto \(A \in \mathbf{Obj}\)
      corresponde al morfismos \(\mathcal i _A\).
  \end{itemize}
\end{definicion}
De la segunda propiedad se extiende que el morfismos identidad es único
para cada \(O \in \mathbf {Obj}\).
\begin{definicion}
  Sean \(\left(  \mathscr{C} , (\circ) \right), \left(\mathscr D ,
    (\star) \right) \) dos categorías con sus respectivos operadores composicion. Un
  ...
  \item  \(\forall f \in \mathbf{Obf} (\mathcal C)\) morfismos de \(X,Y \in \mathbf{Obj}
    (\mathscr C),\ f \in \mathbf{Mor} (\mathscr C)\), se tiene que \(
    F(f) : F(X) \to F(Y) \). Mas aun, \(F (\mathbf{Id}_X) = \mathbf{Id}_{F(X)}\)
  \end{itemize}
\end{definicion}
\begin{acotacion}
  Un functor \(F\) entre las categorias \(\mathcal C\) y \( \mathcal D\)
  debe cumplir que para todo \(f,g \in \mathbf {Obj} \left( \mathcal C
  \right) \) tal que \( f \circ g \in \mathbf{Obj} \left( \mathcal C
  \right)\) se cumple que
...
grupos. Para especificar formalmente nuestro functor \(F\), debemos
especificar tanto el mapeo de los elementos como el de los morfismos.
Estos están dados por las siguientes ecuaciones
\begin{align*}
  F : \mathscr{Top}_* &\longrightarrow \mathscr{Grp} \\
  \forall (A, a_0) \in \mathbf{Obj} \left( \mathscr{Top}_* \right),\
    &F(A) = \pi (A, a_0) \\
  \forall f \in \mathbf{Mor} \left( \mathscr{Top}_* \right),\ &F(f) =
    f_*
\end{align*}
